drop function if exists public.record_my_study_event(text, boolean, integer, text, date, text, uuid, text);
drop function if exists public.record_study_event(uuid, text, boolean, integer, text, date, text, uuid, text);
...
declare
    ...
    v_last_reviewed timestamptz := v_occurred_at;
    ...
    v_effective_log_date date := coalesce(p_log_date, timezone('utc', v_occurred_at)::date);
