...
begin
    ...
    if p_occurred_at is not null and p_occurred_at not between v_received_at - interval '12 hours' and v_received_at + interval '5 minutes' then
        raise exception 'Quiz attempt timestamp is outside the allowed replay window.'
            using errcode = '22023';
    end if;
    ...
    if found then
        if v_existing_attempt.term_id is distinct from p_term_id
           or v_existing_attempt.is_correct is distinct from p_is_correct
           or coalesce(v_existing_attempt.response_time_ms, 0) is distinct from v_response_time_ms
           or v_existing_attempt.quiz_type is distinct from p_quiz_type
           or v_existing_attempt.session_id is distinct from v_effective_session_id
           or (
               p_occurred_at is not null
               and v_existing_attempt.created_at is distinct from v_occurred_at
           ) then
    ...
    if v_inserted_attempt_id is null then
        ...
        if found and (
            v_existing_attempt.term_id is distinct from p_term_id
            or v_existing_attempt.is_correct is distinct from p_is_correct
            or coalesce(v_existing_attempt.response_time_ms, 0) is distinct from v_response_time_ms
            or v_existing_attempt.quiz_type is distinct from p_quiz_type
            or v_existing_attempt.session_id is distinct from v_effective_session_id
